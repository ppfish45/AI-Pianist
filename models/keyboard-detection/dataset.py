...
def show_labelled_image(img, pts):
    if isinstance(img, str):
        img = cv2.imread(img)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    for k, p in enumerate(pts):
        cv2.circle(img, (int(p[0]), int(p[1])), 2, (255 * (k % 3 == 0), 255 * (k % 2 == 0), 255), 2)
    return img

def get_sample(type='train', img=True):
...
class data_batch():
    def __init__(self, type='train', batch_size=64, image_size=(224, 224), NCHW=True, max_num=-1):
        self.type = type
